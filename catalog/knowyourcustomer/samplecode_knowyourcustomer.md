...
Using the KYC (Know Your Customer) API from the backend flow is highly beneficial, as it is a powerful tool for validating identities, authenticating users, and preventing identity theft. It has numerous use cases, such as matching the accuracy of user data during registration by comparing it with the operator’s data, thereby preventing many instances of identity theft. It is also crucial for the financial and insurance sectors, as it facilitates compliance with anti-money laundering (AML) regulations. The authorization protocol used in Open Gateway for backend flows is the OIDC standard CIBA (Client-Initiated Backchannel Authentication). You can check the CAMARA documentation on this flow [here](https://github.com/camaraproject/IdentityAndConsentManagement/blob/release-0.1.0/documentation/CAMARA-API-access-and-user-consent.md#ciba-flow-backend-flow).
